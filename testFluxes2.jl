...
##  2 - RIEMANN
	...
	while( ni <= N_iterations && calcTime <= Total_time_duration )
		...
		elseif (fluxApproxMethod == 1)
			...
			UFLUX = AUSM1dPlusUp(NCells,UconsCellsOld,Gamma); ## Doesn't work yet!!
			
			#UFLUX = AUSM1d(UphysCells,Gamma);
			#UFLUX = UFLUX.*-1.0;
...
x1, theorySolution = computeExactSod1D(198);
x2, RiemannSolution = computeSod1D(198, 2); 
x3, RoeSolution = computeSod1D(198, 0);
#x4, AUSMSolution = computeSod1D(198, 1);
...
plot(x2, RiemannSolution[:,1],"-.r",label="Riemann");
plot(x3, RoeSolution[:,1],"--g",label="Roe");
#plot(x4, AUSMSolution[:,1],"--b",label="AUSM+");
xlabel("x")
ylabel("rho")
xlim(0.0,1.0);
legend();
...
plot(x2, RiemannSolution[:,2],"-.r",label="Riemann");
plot(x3, RoeSolution[:,2],"--g",label="Roe");
#plot(x4, AUSMSolution[:,2],"--b",label="AUSM+");
xlabel("x")
ylabel("U")
xlim(0.0,1.0);
...
plot(x2, RiemannSolution[:,3],"-.r",label="Riemann");
plot(x3, RoeSolution[:,3],"--g",label="Roe");
#plot(x4, AUSMSolution[:,3],"--b",label="AUSM+");
xlabel("x")
ylabel("P")
xlim(0.0,1.0);
...
function auxTestFluxes()
	...
	UFLUX1 = ROE1dtmp(UconsCellsOld,Gamma);
	UFLUX2  =  GOD1d(200,UphysCells,Gamma); #fluxes are PHS variables	
	UFLUX3 = AUSM1dPlusUp(NCells,UconsCellsOld,Gamma);
	UFLUX4 = AUSM1d(UphysCells,Gamma);
